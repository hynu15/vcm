"""
RD curves – H.265/HEVC group.

Methods compared:
  1. Baseline H.265   – full frame, single CRF
  2. SA-CCNet-x265    – SAC with CCNet ROI mask, encoded with x265
  3. SA-PIDNet-x265   – SAC with PIDNet-L ROI mask, encoded with x265
...
Usage:
  ...
  conda run -n sac python3 new_feature/rd_x265.py --num-frames 1
"""
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    out_dir  = PROJECT_ROOT / "outputs" / "new_feature" / "rd_x265" / ts
    ...
    for op in OPERATING_POINTS:
        # ── 1. Baseline H.265 ──────────────────────────────────────────────
        job += 1
        print(f"\n[{job}/{total_jobs}] op={op.label}  Baseline H.265 (crf={op.crf_trad})")
        ...
        # ── 2. SA-CCNet x265 ───────────────────────────────────────────────
        job += 1
        print(f"\n[{job}/{total_jobs}] op={op.label}  SA-CCNet-x265 (crf_roi={op.crf_roi}, crf_non={op.crf_non})")
        ...
        # ── 3. SA-PIDNet x265 ─────────────────────────────────────────────
        job += 1
        print(f"\n[{job}/{total_jobs}] op={op.label}  SA-PIDNet-x265 (crf_roi={op.crf_roi}, crf_non={op.crf_non})")
        dec_bgr, total_b, roi_b, non_b = _encode_decode_sac(
...
def _save_outputs(rows: List[dict], out_dir: Path, args: argparse.Namespace) -> None:
    ...
    _plot(rows, out_dir / "rd_miou.png",
          ...
          title="Rate–Accuracy: H.265/HEVC group")
    _plot(rows, out_dir / "rd_sa_psnr.png",
          metric="sa_psnr", ylabel="SA-PSNR (dB)",
          title="Rate–Quality (SA-PSNR): H.265/HEVC group")
...
def _plot(rows: List[dict], out_png: Path, metric: str, ylabel: str, title: str) -> None:
    ...
    styles = {
        "H.265":     {"color": "#1f77b4", "marker": "o", "ls": "-"},
